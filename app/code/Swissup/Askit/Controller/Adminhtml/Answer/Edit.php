...
class Edit extends Action
{
    ...
    public function execute()
    {
        ...
        $modelQuestion = $this->_objectManager->create('Swissup\Askit\Model\Message');
        if ($parentId) {
            $modelQuestion->load($parentId);
            ...
            $model->setParentId($modelQuestion->getId());

            $adminUser = $this->backendAuthSession->getUser();
            $model->setCustomerName($adminUser->getFirstname() . ' ' . $adminUser->getLastname())
                ->setEmail($adminUser->getEmail())
                ->setStatus(Message::STATUS_APPROVED)
            ;
        }

        $this->coreRegistry->register('askit_answer', $model);
        ...
        if ($id) {
            $resultPage->getConfig()->getTitle()->prepend(__('Askit Items'));
            $resultPage->getConfig()->getTitle()->prepend($model->getText());
        } else {
            $resultPage->getConfig()->getTitle()->prepend('Question : ' . $modelQuestion->getText());
        }
